...
function ConfirmPresence() {
  ...
  const combinedGodfather =
    mainGuest?.isGodfather ||
    extraNames.some((n) => findGuest(n)?.isGodfather);

  const combinedGodmother =
    mainGuest?.isGodmother ||
    extraNames.some((n) => findGuest(n)?.isGodmother);

  const getButtonLabel = () => {
    if (!mainGuest) return "Confirmar presença";

    if (combinedGodfather && combinedGodmother)
      return "Confirmar como Padrinho e Madrinha 💙";

    if (combinedGodfather)
      return "Confirmar como Padrinho 💙";

    if (combinedGodmother)
      return "Confirmar como Madrinha 💙";
  ...
  const getInputBorder = (status) => {
    if (status === "valid")
      return "border-green-400 ring-2 ring-green-300 bg-green-50/30";

    if (status === "invalid")
      return "border-red-300 ring-2 ring-red-200 bg-red-50/30";
  ...
  const handleSubmit = async (e) => {
    ...
    if (!guest) {
      setErrorMsg(
        "Seu nome não foi encontrado. Verifique se escreveu corretamente."
      );
      return;
    }
    ...
    try {
      ...
      if (guest.isGodfather || guest.isGodmother) {
        vipList.push(guest);
      }
      ...
      for (const extra of extraNames) {
        if (extra.trim()) {
          ...
          if (
            extraGuest &&
            (extraGuest.isGodfather || extraGuest.isGodmother)
          ) {
            vipList.push(extraGuest);
          }
        }
      }
      ...
      setShowManual((guestsCount - vipList.length) > 0);
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/confirm/decline`, {
        name: guest.name,
      });
    ...
    <>
      ...
      >
        <img
          src={bgConfirm}
          alt="background"
          className="absolute w-full h-full object-cover blur-lg scale-110 opacity-40"
        />

        <div className="absolute w-full h-full bg-[#FAF9F6]/80 backdrop-blur-sm" />

        <div className="relative z-10 w-full flex flex-col items-center text-center">
          <h2 className="text-5xl md:text-7xl font-display mb-6 text-[#1a3a5c]">
            Confirmar Presença
          </h2>
          ...
          <div className="bg-white/90 backdrop-blur-md rounded-[2rem] p-8 max-w-xl w-full mb-12 shadow-xl border border-white/50 flex flex-col gap-5 text-left">
            <div className="flex items-center gap-4 text-[#1a3a5c]">
              <div className="bg-[#6FAED9]/20 p-3 rounded-full">
                <FaClock className="text-[#6FAED9]" />
              </div>
            ...
            <div className="flex items-center gap-4 text-[#1a3a5c]">
              <div className="bg-[#6FAED9]/20 p-3 rounded-full">
                <FaUserTie className="text-[#6FAED9]" />
              </div>
          ...
          {!confirmed && !declined && (
            <form
              ...
              className="bg-white p-10 rounded-[2.5rem] shadow-2xl w-full max-w-lg flex flex-col gap-6 text-left"
            >
              {/* Nome */}
              <div className="relative">
                <input
                  ...
                  className={`w-full px-5 py-4 rounded-2xl border outline-none font-serif text-lg ${getInputBorder(
                    nameStatus
                  )}`}
                />
                ...
                {nameStatus === "invalid" && (
                  <p className="text-red-400 text-[10px] absolute -bottom-5 left-2 italic font-bold">
                    Nome não encontrado na lista oficial
                  </p>
                )}
              </div>
              ...
              <div className="mt-2 text-left">
                <label className="text-[10px] uppercase tracking-widest text-[#1a3a5c]/50 mb-2 block ml-2 font-bold font-sans">
                  Quantas pessoas?
                </label>
                ...
                <select
                  ...
                  className="w-full px-5 py-4 rounded-2xl border border-gray-100 outline-none focus:border-[#6FAED9] bg-gray-50/30 font-serif text-lg"
                >
              ...
              {extraNames.map((extra, index) => (
                <div key={index} className="relative animate-fadeIn">
                  <input
                    ...
                    className={`w-full px-5 py-4 rounded-2xl border outline-none font-serif text-lg ${getInputBorder(
                      extraNamesStatus[index]
                    )}`}
                  />

                  <div className="absolute right-5 top-1/2 -translate-y-1/2">
                    {extraNamesStatus[index] === "valid" && (
                      <span className="text-green-500 font-bold text-xl">
                        ✓
                      </span>
                    )}
              ...
              {errorMsg && (
                <p className="text-red-400 text-sm italic text-center">
                  {errorMsg}
                </p>
              )}
              ...
              <div className="flex flex-col gap-4">
                <button
                  type="submit"
                  disabled={loading}
                  className={`text-white py-5 rounded-full font-bold text-lg shadow-lg ${
                    loading
                      ? "bg-gray-300"
                      : "bg-[#6FAED9] hover:bg-[#1a3a5c] shadow-[#6FAED9]/20"
                  }`}
                >
                  {loading
                    ? "Processando..."
          ...
          {confirmed && (
            <div className="bg-white p-12 rounded-[2.5rem] shadow-2xl text-center flex flex-col items-center">
              ...
              <h3 className="text-3xl font-display text-[#1a3a5c] mb-2">
                Presença Confirmada!
              </h3>
          ...
          {declined && (
            <div className="bg-white p-12 rounded-[2.5rem] shadow-2xl text-center flex flex-col items-center">
              ...
              <p className="font-serif italic text-gray-500 text-lg">
                Agradecemos por avisar. Sentiremos sua falta! 💙
              </p>
            </div>
          )}
        </div>
          ...
          {vipGuests.length > 0 &&
            vipGuests.map((g, i) => (
              <GodfatherArea
                key={i}
                name={g.name}
                isGodmother={g.isGodmother || false}
              />
            ))}
